...
public class BoardService {
    ...
    public void initiateFleet(Fleet fleet, String gameId, int playerId) {
        Board matchingBoard = repository.getSpecifiedBoards(gameId, playerId).get(0);
        for (int must : fleet.retrieveMastsCellIDs()) {
            matchingBoard.getCells().put(must, Cell.MAST_ALIVE);
        }
